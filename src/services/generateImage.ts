import type { ExKink, ExKinkCategory, InKinkCategory } from '@/models/kinks';
...
const stripIds = (data: InKinkCategory[]): ExKinkCategory[] => {
  return data.map(
    (inCat): ExKinkCategory => ({
      name: inCat.name,
      subcategories: inCat.subcategories.slice(),
      kinks: inCat.kinks.map(
        (inKink): ExKink => ({
          name: inKink.name,
...
export const generateKinklistImage = (
  inCategories: InKinkCategory[],
  ratings: Rating[],
  username: string,
  encodeData: boolean
): HTMLCanvasElement => {
...
const getCanvasDimensions = (
  ...
  numDataPixels: number
): { canvasWidth: number; canvasHeight: number } => {
  const canvasWidth = config.offsetLeft + config.numCols * config.categoryWidth + config.offsetRight;
  const canvasHeight =
    config.offsetTop + tallestColumnHeight + config.offsetBottom + Math.ceil(numDataPixels / canvasWidth);
  return {
    canvasWidth,
    canvasHeight
  };
...
const addUsernameToCanvas = (context: CanvasRenderingContext2D, username: string): void => {
  ...
  context.fillText(`Kinklist (v.Rsp 1) ` + username, 10, 50);
};

const divideCategoryColumns = (
  categories: ExKinkCategory[]
): { columns: ExKinkCategory[][]; tallestColumnHeight: number } => {
...
const calculateCategoryHeight = (category: ExKinkCategory): number => {
  const titleHeight =
    category.subcategories.length > 1 ? config.titleWithSubcategoryHeight : config.titleWithoutSubcategoryHeight;
...
const drawCategory = (
  ...
  for (let i = 0; i < category.kinks.length; i++) {
    ...
    const rowY =
      y +
      (hasSubtitle ? config.titleWithSubcategoryHeight : config.titleWithoutSubcategoryHeight) +
      i * config.categoryRowHeight;
    // Text
    context.fillStyle = '#000000';
    context.font = '24px Arial';
    context.fillText(kink.name, x + 10 + category.subcategories.length * 50, rowY - 6);
    // Circles
    for (let n = 0; n < category.subcategories.length; n++) {
      const subcategory = category.subcategories[n];
      let rating = ratings.find((r) => r.name === kink.ratings[subcategory]) as Rating;
      if (!rating) {
        rating = { name: 'NaN', color: 'grey' };
      }
      const cx = x + 20 + n * 40;
      const cy = rowY - 20;
